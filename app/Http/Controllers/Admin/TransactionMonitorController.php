...
class TransactionMonitorController extends Controller
{
  ...
  /**
     * Customer Support queue — simplified to Open + Resolved view.
     */
    public function customerSupport(Request $request)
    {
        ...
        $perPage = 20;
        $page = (int) $request->query('page', 1);
        $category = $request->query('category', 'open'); // open | resolved
        $search = $request->query('search', '');

        $query = Transaction::with('user:id,name,email,profile_picture', 'resolver:id,name')
            ->with('resolver:id,name');

        // Category filter
        if ($category === 'open') {
            // Failed OR pending OR flagged AND not resolved
            $query->where(function ($q) {
                $q->whereIn('status', ['failed', 'pending'])
      ...
      // Smart sort: For Open cases, latest reported first. For Resolved, latest resolved first.
            if ($category === 'resolved') {
                $query->orderBy('resolved_at', 'desc');
            } else {
            ...
            $transactions = $query
                ...
                return [
                    ...
                    'created_at' => $t->created_at?->toIso8601String(),
                    'created_relative' => $t->created_at?->diffForHumans() ?? '—',
                    'created_short' => $t->created_at?->format('M j, g:i A') ?? '—',
                    'resolved_at' => $t->resolved_at?->toIso8601String(),
                    'resolved_relative' => $t->resolved_at?->diffForHumans(),
                    'resolved_at_short' => $t->resolved_at?->format('M j, g:i A'),
                    'resolver_name' => $t->resolver?->name,
                ];
            });

        $openCount = Transaction::where(function ($q) {
            $q->whereIn('status', ['failed', 'pending'])->orWhere('is_flagged', true);
        })->where('is_resolved', false)->count();

        $counts = [
            'open' => $openCount,
            'resolved' => Transaction::where('is_resolved', true)->count(),
        ];

        $stats = [
            'open_cases' => $openCount,
            'resolved_today' => Transaction::where('is_resolved', true)
                ->whereDate('resolved_at', today())
                ->count(),
        ];

        return Inertia::render('Admin/CustomerSupport', [
            'auth' => ['user' => $admin],
            'transactions' => $transactions,
            'pagination' => [
                'current_page' => $page,
    ...
    {
        $resolved = Transaction::where('is_resolved', true)
        ...
        $totalHours = $resolved->sum(function ($t) {
