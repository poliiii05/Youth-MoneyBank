...
import { Target, CreditCard, Settings, Menu, Home, Bell, Sparkles, Star, X } from 'lucide-react';
export default function UserLayout({ user, header, children }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50/50">
            ...
            {/* SIDEBAR — simplified, navigation only */}
            <aside className={`
                fixed top-0 left-0 h-full w-64 bg-white border-r border-slate-300 z-50 transform transition-transform duration-300
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <img 
                                src="/images/logo.png" 
                                alt="Logo" 
                                className="w-9 h-9 object-contain rounded-xl shadow-sm border border-slate-300" 
                                onError={(e) => {
            ...
            <div className="lg:ml-64 flex flex-col min-h-screen">
                ...
                {/* HEADER — TeraBox-style with profile dropdown */}
                <header className="bg-white/95 backdrop-blur-md sticky top-0 z-30 border-b border-slate-300 shadow-sm">
                    <div className="flex items-center justify-between px-4 py-3 lg:px-8">
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                className="lg:hidden p-2 hover:bg-slate-100 text-slate-600 rounded-lg transition-colors cursor-pointer"
                            >
                                <Menu className="w-5 h-5" />
                            </button>
                            <h2 className="text-base font-bold text-slate-900 hidden sm:block tracking-tight">
                                {header || "Dashboard"}
                            </h2>
                        </div>

                        {/* RIGHT: Upgrade + Bell + Profile */}
