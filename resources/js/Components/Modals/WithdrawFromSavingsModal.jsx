...
import { X, Wallet, PiggyBank, AlertCircle, Loader2, Sparkles, ArrowRight, ArrowUpRight } from 'lucide-react';
...
export default function WithdrawFromSavingsModal({ isOpen, onClose, savingsPoolBalance = 0 }) {
    ...
    const handleSetMax = () => {
        setAmount(savingsPoolBalance.toLocaleString('en-US'));
        setErrorMsg('');
    };
            ...
            <div className="bg-white w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300 relative max-h-[90vh] flex flex-col">
                ...
                {isProcessing && (
                    <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur-md flex flex-col items-center justify-center">
                        <div className="relative mb-4">
                            ...
                            <Loader2 className="w-8 h-8 text-slate-700 animate-spin absolute inset-0 m-auto" strokeWidth={2.5} />
                        </div>
                        <h3 className="text-sm font-black text-slate-900">Withdrawing</h3>
                    </div>
                )}

                {/* HERO HEADER — slate (neutral action) */}
                        ...
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            ...
                            <div className="min-w-0">
                                ...
                                <h2 className="text-base font-black text-white tracking-tight leading-tight truncate">
                                    {isSuccess ? 'Withdrawal complete!' : 'Withdraw to Wallet'}
                                </h2>
                            </div>
                        </div>
                ...
                {isSuccess ? (
                    <div className="px-5 py-6 flex flex-col items-center text-center">
                        ...
                        <h3 className="text-lg font-black text-slate-900 mb-1">Funds moved!</h3>
                        <p className="text-xs text-slate-500 font-medium mb-5">
                            <span className="font-bold text-slate-800" style={{ fontVariantNumeric: 'tabular-nums' }}>₱{numericAmount.toLocaleString('en-US')}</span> moved to your main wallet 🎉
                        </p>
                    ...
                    <div className="px-5 py-8 flex flex-col items-center text-center">
                        <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-3">
                            <PiggyBank size={32} className="text-slate-400" strokeWidth={2} />
                        </div>
                        <h3 className="text-base font-black text-slate-900 mb-1">Nothing to withdraw</h3>
                        <p className="text-xs text-slate-500 font-medium mb-5">
                            Your savings pool is empty. Add funds first 💰
                        </p>
                        <button onClick={onClose} className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black rounded-xl transition-all cursor-pointer active:scale-[0.98]">
                            Got it
                        </button>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto px-5 py-5">
                            ...
                            {/* Flow visualization */}
                            <div className="flex items-center justify-between gap-3 mb-5 p-3 bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-xl border border-slate-200">
                                ...
                                <div className="flex-1 text-center min-w-0">
                                    ...
                                    <p className="text-sm font-black text-slate-700" style={{ fontVariantNumeric: 'tabular-nums' }}>
                            ...
                            <div className="mb-3">
                                <div className="flex items-center justify-between mb-1.5">
                                    ...
                                    <button
                                        ...
                                        className="text-[10px] font-black text-slate-700 hover:text-slate-900 uppercase tracking-widest cursor-pointer active:scale-95 transition-all"
                                    >
                                        Use Max
                                    </button>
                                </div>
                                    ...
                                    <input 
                                        ...
                                        className={`w-full pl-11 pr-3 py-3.5 border-2 rounded-xl text-xl font-black outline-none transition-all ${
                                            hasErrorUI 
                                                ...
                                                : 'bg-white border-slate-200 focus:border-slate-500 focus:ring-4 focus:ring-slate-100 text-slate-900 placeholder:text-slate-300'
                                        }`} 
                                    />
                                </div>
                                            ...
                                            <button 
                                                ...
                                                className={`py-2.5 rounded-xl text-xs font-black border-2 transition-all duration-150 active:scale-95 ${
                                                    isDisabled 
                                                        ...
                                                        : isSelected
                                                            ? 'bg-slate-900 text-white border-slate-900 shadow-md shadow-slate-300 scale-105 cursor-pointer'
                                                            : 'bg-white text-slate-700 border-slate-200 hover:border-slate-400 hover:bg-slate-50 hover:-translate-y-0.5 cursor-pointer'
                                                }`}
                                            >
                                                ₱{preset}
                                            </button>
