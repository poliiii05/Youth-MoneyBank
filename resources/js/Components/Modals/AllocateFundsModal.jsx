...
import { X, PiggyBank, Target, AlertCircle, Loader2, Sparkles } from 'lucide-react';
...
export default function AllocateFundsModal({ isOpen, onClose, goal, savingsPoolBalance = 0 }) {
    ...
    // Computations (safe defaults kahit goal is null)
    ...
    // Global Enter key handler — must be before any early return
    useModalEnterKey({
        isOpen,
        isSuccess,
            ...
            <div className="bg-white w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 relative">
                {isProcessing && (
                    <div className="absolute inset-0 z-50 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center">
                        ...
                        <h3 className="text-sm font-bold text-slate-900">Allocating Funds</h3>
                    </div>
                )}
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
                    <div className="w-7 h-7" />
                    <h2 className="text-[13px] font-bold text-slate-900">{isSuccess ? 'Allocation complete' : 'Add funds to goal'}</h2>
                ...
                <div className="max-h-[80vh] overflow-y-auto">
                    {isSuccess ? (
                        <div className="px-5 py-6 flex flex-col items-center text-center">
                            ...
                            <h3 className="text-base font-bold text-slate-900 mb-1">Funds allocated!</h3>
                            <p className="text-[11px] text-slate-500 mb-4">
                                <span className="font-bold text-slate-800">₱{numericAmount.toLocaleString('en-US')}</span> added to '{goal.title}'.
                            </p>
                        ...
                        <div className="px-5 py-5">
                            <div className="mb-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500">{((goal.current_amount / goal.target_amount) * 100).toFixed(0)}% done</span>
                                </div>
                            ...
                            <div className="mb-4 flex items-center justify-between px-3 py-2 bg-emerald-50/50 border border-emerald-100 rounded-lg">
                                ...
                                <span className="text-xs font-black text-emerald-900">₱{savingsPoolBalance.toLocaleString('en-US')}</span>
                            </div>
                            {isGoalFull ? (
                                ...
                                <>
                                    <div className="mb-2">
                                        <div className="flex items-center justify-between mb-1.5">
                                            <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest">Amount to allocate</label>
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                                Max: <span className={`${hasErrorUI ? 'text-red-500' : 'text-emerald-600'}`}>₱{maxAllowable.toLocaleString('en-US')}</span>
                                            </span>
                                        </div>
                                        <div className="relative">
                                            <span className={`absolute left-3 top-1/2 -translate-y-1/2 font-semibold text-sm ${hasErrorUI ? 'text-red-400' : 'text-slate-400'}`}>₱</span>
                                            <input type="text" inputMode="numeric" value={amount} onChange={handleAmountChange} placeholder="Min. 1" autoFocus
                                                className={`w-full pl-7 pr-3 py-2 border rounded-xl text-base font-semibold outline-none transition-all ${hasErrorUI ? 'bg-red-50/20 border-red-400 focus:ring-4 focus:ring-red-50 text-red-900' : 'bg-white border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-50 text-slate-900'}`} />
                                        </div>
                                        <div className={`min-h-[16px] mt-1.5 transition-all duration-300 ${hasErrorUI ? 'opacity-100' : 'opacity-0'}`}>
                                            {hasErrorUI && <p className="text-[10px] font-semibold text-red-500 flex items-center gap-1"><AlertCircle size={10} /> {errorMsg}</p>}
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-4 gap-2 mb-5">
                                        {PRESET_AMOUNTS.map((preset) => {
                                            ...
                                            return (
                                                <button key={preset} type="button" onClick={() => handlePreset(preset)} disabled={isDisabled}
                                                    className={`py-1.5 rounded-lg text-[10px] font-bold border transition-all ${isDisabled ? 'bg-slate-50 text-slate-300 border-slate-100 cursor-not-allowed' : numericAmount === preset ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400 cursor-pointer'}`}>
                                                    ₱{preset}
                                                </button>
                                            );
                                        })}
                                    </div>
                                    <button onClick={handleSubmit} disabled={!isAmountValid || isProcessing}
                                        className={`w-full py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${isAmountValid && !isProcessing ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-200' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
