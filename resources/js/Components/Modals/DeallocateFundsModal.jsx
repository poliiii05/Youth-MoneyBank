...
import { X, PiggyBank, Target, AlertCircle, Loader2, Sparkles, ArrowRight } from 'lucide-react';
...
export default function DeallocateFundsModal({ isOpen, onClose, goal }) {
    ...
    const maxAllowable = goal.current_amount;
    ...
    const handleSubmit = () => {
        ...
        router.post(`/goals/${goal.id}/deallocate`, {
            ...
            <div className="bg-white w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 relative">
                ...
                {isProcessing && (
                    <div className="absolute inset-0 z-50 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-300">
                        ...
                        <h3 className="text-sm font-bold text-slate-900 tracking-tight mb-1">Moving Funds Back</h3>
                    </div>
                )}
                ...
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
                    ...
                    <h2 className="text-[13px] font-bold text-slate-900 tracking-tight">
                        {isSuccess ? 'Unallocation complete' : 'Unallocate Funds'}
                    </h2>
                    <button
                        onClick={onClose}
                    ...
                    {isSuccess ? (
                        <div className="px-5 py-6 flex flex-col items-center text-center">
                            ...
                            <p className="text-[11px] text-slate-500 mb-4">
                                <span className="font-bold text-slate-800">₱{numericAmount.toLocaleString('en-US')}</span> moved from '{goal.title}' to your savings pool.
                            </p>
                            <button
                                onClick={onClose}
                        ...
                        <div className="px-5 py-5">
                            ...
                            <div className="flex items-center justify-between gap-3 mb-5 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                <div className="flex-1 text-center">
                                    ...
                                    <p className="text-sm font-black text-slate-900">₱{goal.current_amount.toLocaleString('en-US')}</p>
                                </div>
                            ...
                            <div className="mb-2">
                                <div className="flex items-center justify-between mb-1.5">
                                    <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                        Amount to unallocate
                                    </label>
                                    <button
                                        type="button"
                                        onClick={handleSetMax}
                                        className="text-[9px] font-bold text-blue-600 hover:text-blue-700 uppercase tracking-widest cursor-pointer"
                                    >
                                        Use Max
                                    </button>
                                </div>
